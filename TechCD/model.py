...
from gnn import GCN  # 使用之前重写的 GCN 类
...
class Net(nn.Module):
    ...
    def __init__(self, student_n, exer_n, knowledge_n, local_map, device):
        ...
        # 创建 GCN 层
        self.similarity_gcn = self.create_gcn_layers(self.num_layers)
        self.prerequisite_gcn = self.create_gcn_layers(self.num_layers)
        self.exer_concept_gcn = self.create_gcn_layers(self.num_layers)
        ...
        self.prednet_full3 = nn.Linear(self.prednet_input_len, 1)
    ...
    def create_gcn_layers(self, num_layers):
        gcn_layers = nn.ModuleList()
        for _ in range(num_layers):
            gcn_layers.append(GCN(self.knowledge_dim, self.knowledge_dim, None))
        return gcn_layers

    def forward(self, stu_id, exer_id, kn_emb, input_knowedge_ids, history, emb):
        '''
        :param:
        ...
        # 图数据输入
        similarity_data = Data(x=entity_embeddings[-1], edge_index=self.similarity_g.edge_index)
        prerequisite_data = Data(x=entity_embeddings[-1], edge_index=self.prerequisite_g.edge_index)
        exer_concept_data = Data(x=entity_embeddings[-1], edge_index=self.exer_concept_g.edge_index)
        ...
        for similarity_gcn_layer, prerequisite_gcn_layer, exer_concept_gcn_layer in zip(self.similarity_gcn, self.prerequisite_gcn, self.exer_concept_gcn):
            similarity_embeddings = similarity_gcn_layer(similarity_data)
            prerequisite_embeddings = prerequisite_gcn_layer(prerequisite_data)
            exer_concept_embeddings = exer_concept_gcn_layer(exer_concept_data)
